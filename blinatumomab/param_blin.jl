# author: Yuezhe LI
# date: 11/1/2024
# purpose of this code: to define parameters for blin_ode.jl (bincyto parameters for Hosseini model)
...
p_homo_blin = ComponentArray(
    ...
    KmBT_act_blin = 0.72,           # The B/T rario, at which the rate is half of VmT 
    S = 1.4,                        # Hill coefficient for the B:T portion of the Michaelis-Menten equation
    KdrugactT_blin = 0.1,           # conc at which T-cell activation was half of VmT [ng/mL]
    # TDB : B-cell kill
    VmB = 0.95,                     # maximum rate of B-cell killing 
    KmTB_kill_blin = 0.75,          # the Tact/B ratio that the rate if half of VmB
    ...
    nkill_blin = 1.03,              # Hill coefficient for the Tact/B portion of the Michaelis-Menten equation
    KdrugB_blin = 0.15,             # the TDB conc at which B-cell killing rate if half of VmB [ng/mL]
    #
    # B-cells
    ...
    tumor_on = 1.,                 # this turns on/ off the tumor compartment 
    ...
    fbio_TDB = 0.7, 
    # blinatumomab PK 
    Cl_blin = 22300.00, # [ml/m2/day]
    Vz_blin =  1610.00,  # [ml/m2]
    blin_infusion = 0  # [ug/m2/d]
);
