# date: 9/13/24
# author: Yuezhe Li 
# purpose of this code: to remove T cells in spleen & BM, remove B cells in spleen

using DifferentialEquations, ModelingToolkit 

include("param.jl");

function TDB_homo6(; name)
    ...
    eqs = [
        ...
        Dt(Bpb) ~ ( tissue3on*kBapop*Bpb + tissue3on*kBprolif*Bpbref_perml*Vpb*max(0,1-Bpb/(Bpbref_perml*Vpb)) - 
                    kBapop*Bpb - kBkill*drugBpbkill*Bpb - B_enter_LN + B_leaving_BM ), 
        ...
        Dt(B19no20tiss3) ~ ( (Bpbref_perml*KBp3*B19no20_B1920_ratio*Vtissue3*(kBapop+(kBapop/kBmat_kBapop_ratio))*(1+5*(max(0,1-(Bpb+Btiss2)/(Bpbref_perml*Vpb+KBp2*Bpbo_perml*Vtissue2)))^1)) - ((kBapop/kBmat_kBapop_ratio)*B19no20tiss3) - (kBapop*B19no20tiss3) + (kBprolif*KBp3*Bpbref_perml*B19no20_B1920_ratio*Vtissue3*(max(0,1-B19no20tiss3/(KBp3*Bpbref_perml*B19no20_B1920_ratio*Vtissue3)))^1)),
