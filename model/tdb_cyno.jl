# date: 8/30/24
# this code is based on https://ghe.metrumrg.com/yuezhel/hosseini-2020/blob/main/hosseini-cyno-model-salad.jl
# this version further cleaned up some parameters that were not documented in Hosseini model and with value of 1

using DifferentialEquations, ModelingToolkit 

include("param.jl");

function TDB_ADC_homo(; name)
    ...
    pars = @parameters begin 
        ...
        infusion_TDB = 0.
        # ADC-related param
        CL_ADC = 0.015e3*DayToHour 
        V1_ADC = 3.1e3 
        V2_ADC = 3.13e3 
        Q_ADC = 0.13e3*DayToHour 
        infusion_ADC = 0.0 
        ADC_kill = 0.05 * 0.15 * DayToHour 
        ADC_EC50 = 1.76E-5 
        n_kill_ADC = 0.708
        k_trans = 0.05
        ADC_kill_neg = 0. 
        k_trans_neg = 0.02
        ADC_EC50_neg = 1.76E-3
    end 
    ...
    vars = @variables begin 
        ...
        ## Tumor
        Btumor(t) = Init_Vtumor*0.375/Vc*(1-Init_cd19neg_tumor)
        Btumor_trans(t) = 0.
        Btumor_cd19neg(t) = Init_Vtumor*0.375/Vc*Init_cd19neg_tumor
        Btumor_cd19neg_trans(t) = 0.
        ## IL6
        IL6pb(t) = 0.
        IL6tiss(t) = 0.
        IL6tiss2(t) = 0.
    ...
    Vtumor = Vc * max((Btumor + Btumor_trans + Btumor_cd19neg + Btumor_cd19neg_trans), 1.) / 0.375
    ...
    ADC1_ugperml = tissue1on*Kp*max(ADCc_ugperml, 0.)
    ADC2_ugperml = tissue2on*Kp2*max(ADCc_ugperml, 0.)
    ADC3_ugperml = tissue3on*Kp3*max(ADCc_ugperml, 0.)
    ADCtumor_ugperml = tumor_on*Kptumor*max(ADCc_ugperml, 0.)
    # T cells
    ...
    # ADC cytotoxicity 
    ADCBbp = ADC_kill * ADCc_ugperml^n_kill_ADC/(ADCc_ugperml^n_kill_ADC+ADC_EC50^n_kill_ADC)
    ADCBtissueact1 = ADC_kill * ADC1_ugperml^n_kill_ADC/(ADC1_ugperml^n_kill_ADC+ADC_EC50^n_kill_ADC)
    ADCBtissueact2 = ADC_kill * ADC2_ugperml^n_kill_ADC/(ADC2_ugperml^n_kill_ADC+ADC_EC50^n_kill_ADC)
    ADCBtissueact3 = ADC_kill * ADC3_ugperml^n_kill_ADC/(ADC3_ugperml^n_kill_ADC+ADC_EC50^n_kill_ADC)
    ADCBtumoract = ADC_kill * ADCtumor_ugperml^n_kill_ADC/(ADCtumor_ugperml^n_kill_ADC+ADC_EC50^n_kill_ADC)
    ADCBtumoractneg = ADC_kill_neg * ADCtumor_ugperml^n_kill_ADC/(ADCtumor_ugperml^n_kill_ADC+ADC_EC50_neg^n_kill_ADC)
    # diff equations
    eqs = [
        # T cells
        ...
        # PK 
        Dt(TDBc_ugperml) ~ ( infusion_TDB + kabs_TDB * TDBdepot_ug/V1_TDB - 1/V1_TDB * Q_TDB * (TDBc_ugperml - TDBp_ugperml) - CL_TDB/V1_TDB * TDBc_ugperml - Vm_tdb/V1_TDB / (Km_tdb + TDBc_ugperml) * TDBc_ugperml ), 
        Dt(TDBp_ugperml) ~ ( 1/V2_TDB * Q_TDB * (TDBc_ugperml - TDBp_ugperml) ), 
        Dt(TDBdepot_ug) ~ ( - kabs_TDB * TDBdepot_ug ), 
        Dt(ADCc_ugperml) ~ infusion_ADC - 1/V1_ADC * Q_ADC * (ADCc_ugperml - ADCp_ugperml) - CL_ADC/V1_ADC * ADCc_ugperml, 
        Dt(ADCp_ugperml) ~ 1/V2_ADC * Q_ADC * (ADCc_ugperml - ADCp_ugperml),
        # IL6 
        Dt(IL6pb) ~ (
        ...
        Dt(Bpb) ~ ( tissue3on*kBapop*Bpb + tissue3on*kBprolif*Bpbref_perml*Vpb*max(0,1-Bpb/(Bpbref_perml*Vpb)) - 
                    kBapop*Bpb - kBkill*drugBpbkill*Bpb - ADCBbp*Bpb - B_enter_spleen - B_enter_LN + B_leaving_BM ), 
        Dt(Btiss) ~ ( B_enter_spleen - (fBkill*kBkill*drugBtisskill*Btiss) - ADCBtissueact1*Btiss + (kBprolif*KBp*Bpbref_perml*Vtissue*(max(0,1-Btiss/(KBp*Bpbref_perml*Vtissue)))^1) ), 
        Dt(Btiss2) ~ ( B_enter_LN -(fBkill*kBkill*drugBtisskill2*Btiss2) - ADCBtissueact2*Btiss2 + (kBprolif*KBp2*Bpbref_perml*Vtissue2*(max(0,1-Btiss2/(KBp2*Bpbref_perml*Vtissue2)))^1) ), 
        Dt(B1920tiss3) ~ ( kBprolif*KBp3*Bpbref_perml*Vtissue3*max(0,1-B1920tiss3/(KBp3*Bpbref_perml*Vtissue3)) - B_leaving_BM + 
            kBapop/kBmat_kBapop_ratio*B19no20tiss3 - kBapop*B1920tiss3 - fBkill*kBkill*drugBtisskill3*B1920tiss3 - ADCBtissueact3*B1920tiss3 ), 
        Dt(B19no20tiss3) ~ ( - ADCBtissueact3*B19no20tiss3 + (Bpbref_perml*KBp3*B19no20_B1920_ratio*Vtissue3*(kBapop+(kBapop/kBmat_kBapop_ratio))*(1+5*(max(0,1-(Bpb+Btiss+Btiss2)/(Bpbref_perml*Vpb+KBp*Bpbo_perml*Vtissue+KBp2*Bpbo_perml*Vtissue2)))^1)) - ((kBapop/kBmat_kBapop_ratio)*B19no20tiss3) - (kBapop*B19no20tiss3) + (kBprolif*KBp3*Bpbref_perml*B19no20_B1920_ratio*Vtissue3*(max(0,1-B19no20tiss3/(KBp3*Bpbref_perml*B19no20_B1920_ratio*Vtissue3)))^1)),
        Dt(Btumor) ~ ( (kBtumorprolif*Btumor) - (fBkill*kBkill*drugBtumorkill*Btumor + ADCBtumoract*Btumor) ), 
        Dt(Btumor_trans) ~ ( ADCBtumoract*Btumor - k_trans*Btumor_trans ),
        Dt(Btumor_cd19neg) ~ ( (kBtumorprolif*Btumor_cd19neg) - (fBkill*kBkill*drugBtumorkill*Btumor_cd19neg + ADCBtumoractneg*Btumor_cd19neg) ),
        Dt(Btumor_cd19neg_trans) ~ ( ADCBtumoractneg*Btumor_cd19neg - k_trans_neg*Btumor_trans ), 
        # other 
