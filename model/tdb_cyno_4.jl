# date: 9/13/24
# author: Yuezhe Li 
# purpose of this code: to remove T cells in spleen

using DifferentialEquations, ModelingToolkit 

include("param.jl");

function TDB_homo4(; name)
    ...
    eqs = [
        # T cells
        Dt(restTpb) ~ ( T_postact_rest_conv_PB - rest_T_enter_LN - rest_T_enter_BM - rest_T_enter_Tumor - T_act_rest_PB + 
                        (kTgen*Vpb*Trpbref_perml*(max(0,1-totTpb_perml/Trpbref_perml)^2)) - fTrapop*kTaapop*max(0,restTpb-Trpbref_perml*Vpb)  ), 
        Dt(actTpb) ~ ( T_act_rest_PB + T_act_postact_PB - T_act_traffick_LN - T_act_traffick_BM - T_act_traffick_Tumor - (kTaapop*(actTpb+fAICD*actTpb^2/(Vpb*Trpbref_perml))) ), 
        Dt(act0Tpb) ~ ( (act0on*fTaprolif*kTprolif*actTpb) - (fTa0apop*kTaapop*act0Tpb) - T_postact_rest_conv_PB - T_act_postact_PB
                          - postact_T_traffick_LN - postact_T_traffick_BM - postact_T_traffick_Tumor),
